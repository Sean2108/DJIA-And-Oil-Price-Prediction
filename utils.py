...
def inverse_transform_single_column(scaler, y_values, num_variables):
    return scaler.inverse_transform(np.array([[y] + [0] * (num_variables - 1) for y in y_values]))[:, 0]
...
def plot_prediction(scaler, x_test, y_test, models):
    ...
    for name, model, color in models:
        y_predict = model.predict(x_test)
        plt.plot(inverse_transform_single_column(scaler, y_predict, x_test.shape[2]), color=color,label=name + ' Predicted Price')
